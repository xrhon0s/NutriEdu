...
const run = async () => {
  ...
  if (users.pagination.total !== 1 || !catalogs.goals.length || !catalogs.conditions.length || role.rol !== "usuario") {
    throw new Error("Admin management smoke contract returned an unexpected result");
  }
  console.log(JSON.stringify({ ok: true, userSearch: true, catalogs: true, roleChange: true }, null, 2));
};
